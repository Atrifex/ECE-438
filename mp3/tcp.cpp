...
void TCP::receiverSetupConnection()
{
	...
	socklen_t addr_len;
	msg_header_t syn, syn_ack, msg_or_ack;
	...
	if ((numbytes = recvfrom(sockfd, (char *)&msg_or_ack, sizeof(msg_header_t) , 0, (struct sockaddr *)&their_addr, &addr_len)) == -1) {
		perror("recvfrom");
		exit(1);
	}

	if(msg_or_ack.length != ){
